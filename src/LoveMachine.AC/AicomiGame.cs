...
public class AicomiGame: GameAdapter
{
    ...
    protected override IEnumerator UntilReady(object instance)
    {
        ...
        females = hscene.Property<IEnumerable>("_hActorReceivers").Value.Cast<Il2CppSystem.Object>()
            .Select(attacker => Traverse.Create(attacker).Property<Animator>("Animator").Value)
            .Select(anim => anim.gameObject)
            .ToArray();
        penises = hscene.Property<IEnumerable>("_hActorAttackers").Value.Cast<Il2CppSystem.Object>()
            .Select(attacker => Traverse.Create(attacker).Property<Animator>("Animator").Value)
