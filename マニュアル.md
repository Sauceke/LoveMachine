...
[一部の自律型デバイス](#supported-devices)について、以下のゲームに対応しています。
| ゲームタイトル | 開発者 | VR対応 |
|:----|:----|:----|
| [カスタムオーダーメイド3D2] | Kiss | ⭕ |
| [聖騎士リッカの物語] | もぐらソフト | ❌ |
...
[LoveMachine.Experiments]プラグインは、[コイカツ！]と[コイカツ！サンシャイン]で実験的な2つの深度センシングデバイス[Lovense Calor]と[Hotdog]のサポートを追加します。
...
スペースキーは、ゲーム中にすべてのデバイスを停止する停止ボタンとして機能します。デバイスを再起動するにはF8キーを推してください。これらのキー割り当ては、プラグイン設定 > LoveMachine > Kill Switch Settingsで変更できます。
